({
    doInit : function(component, event, helper) {
        ...
        var biAction = component.get("c.getBuyInterests");
        biAction.setParams({ leadId : recordId});
        biAction.setCallback(this, function(response){
            var state = response.getState();
            if (state === 'SUCCESS'){
                if(response.getReturnValue().length !== 0){
                    component.set("v.buyInterests", response.getReturnValue());
                    component.set("v.selectedBiId", response.getReturnValue()[0].Id);
                }

            }
        });
        $A.enqueueAction(biAction);
  ...
  /*      var rtAction = component.get("c.getOpportunityRecordTypes");
        ...
        var checkUserType = component.get("c.isFiberProfile");
    	checkUserType.setCallback(this, function(response) {
          if(response.getState() === "SUCCESS") {
            component.set("v.isFiberUser", response.getReturnValue());
        }});

        $A.enqueueAction(checkUserType);
    },
    ...
    submitFiberLead : function(component, event, helper){
        var recordId = component.get('v.recordId');

        if(recordId !== undefined){
            var action = component.get("c.convertFiberLead");
            var buyInterestId = component.get("v.selectedBiId");
            var dupeId = component.get("v.selectedDupeId");
            var oppName = component.get("v.oppName");
            var recordType = component.get("v.selectedRecordType");
    ...
    biSelected : function (component, event, helper) { 
        var target = event.target || event.srcElement; 
        if(target !== undefined) {  
            component.set("v.selectedBiId", target.dataset.id);
        }   
    },
